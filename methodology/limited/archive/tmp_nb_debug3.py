...
def llA(beta, alpha):
    mu = np.exp(X@beta); a=1/alpha
    return (a*np.log(a/(a+mu)) + y*np.log(mu/(a+mu)) + gammaln(y+a) - gammaln(a) - gammaln(y+1.0)).sum()

# Candidate B: NB1-style (Var = mu(1+alpha)) log-likelihood
def llB(beta, alpha):
    mu = np.exp(X@beta); a=1/alpha
    return (y*np.log(mu) - (y+a)*np.log(mu+alpha) + a*np.log(alpha) + gammaln(y+a) - gammaln(a) - gammaln(y+1.0)).sum()

# Candidate C: Var = mu + alpha*mu^2 but NB1-style log with (mu + alpha*mu^2)
def llC(beta, alpha):
    mu = np.exp(X@beta); a=1/alpha; V = mu + alpha*mu**2
    return (y*np.log(mu) - (y+a)*np.log(V) + a*np.log(alpha) + gammaln(y+a) - gammaln(a) - gammaln(y+1.0)).sum()

# Candidate D: Var = mu + alpha*mu (true NB1 linear) gamma mixture style
def llD(beta, alpha):
    mu = np.exp(X@beta); a=1/alpha
    return (a*np.log(a/(a+mu)) + y*np.log(mu/(a+mu)) + gammaln(y+a) - gammaln(a) - gammaln(y+1.0)).sum()  # same as A
