...
##### v1.2 — Count & limited dependent variable models *(shipped 2026-07-19)*
...
- `nbreg()` (NB1/NB2) — *shipped 2026-07-19* — hand-rolled NB1/NB2 (pyfixest has no `fenegbin`; Stata base `nbreg` has no FE absorption) inside the HDFE IRLS core (mirrors `poisson.py`). `NegBinResult` adds `.alpha()`/`.theta()`/`.irr()`/`.margins()`/`.predict()`/`.tidy()`/`.summary()`. Pooled & FE NB2 coefs/alpha/theta/LL match R `glm.nb` + `fixest::fenegbin` AND Stata `nbreg, dispersion(mean)` to 1e-6; NB1 (`dispersion="mean"`) verified internally. `vcov_backend` toggle (`"fixest"` default / `"stata"`) mirrors poisson. **OPEN gaps (documented as strict `xfail` tests, rule 22, FUTURE_WORK.md):** Stata `nbreg, dispersion(constant)` fits a different Stata-specific MLE (not reproduced); Stata non-clustered OIM SEs diverge ~4% from R/oe (cluster-robust is the validated deliverable). See `methodology/limited/nbreg.md`.
- `ologit()` / `oprobit()` (ordered) — *shipped 2026-07-19* — wrap `statsmodels.miscmodels.ordinal_model.OrderedModel`, polished with an L-BFGS-B pass (gtol=1e-12) so point estimates, cutpoints, OIM SEs, and loglik match Stata `ologit`/`oprobit` (and R `MASS::polr` loglik/SE) to 1e-6. `OrderedResult` adds `.cutpoints`, `.predict(type="class"|"probs")`, `.margins()`. `cov_type` ∈ {nonrobust, HC0, HC1, HC2, HC3}. OPEN gaps (documented as strict `xfail` tests, rule 22): Stata-vs-R coef/cutpoint divergence ~1e-5; HC1 robust SE vs Stata `vce(robust)` ~4e-4. See `methodology/limited/ordered.md` and FUTURE_WORK.md.
...
- `tobit()` — MLE (statsmodels has no Tobit); validate vs R `AER::tobit` (censReg NOT installed) *(shipped 2026-07-19)*
- `feglm` binomial FE absorption — **decision required, NOT in v1.2 as scoped (deferred)**; flag in `FUTURE_WORK.md`.
- First-class `.margins()` / `.predict()`; parity vs Stata `poisson` / `nbreg` / `tobit` / `ologit` / `oprobit`
