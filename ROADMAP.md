...
- [ ] **Roll HAC (`cov_type="HAC"`) out to `iv()`/`did()`/`event_study()`/`staggered_did()`/`gmm()`** — these still lack HAC support (the cross-estimator validation unification deliberately did **not** add it; only the casing alias was added where HAC already existed). Separate scoped passes per the existing rollout sequencing.
