...
YD = {e: df[df["entity"] == e].sort_values("time")["y"].to_numpy(dtype=float) for e in entities}
XD = {e: df[df["entity"] == e].sort_values("time")["x"].to_numpy(dtype=float) for e in entities}
ZD = {e: df[df["entity"] == e].sort_values("time")["z"].to_numpy(dtype=float) for e in entities}
    ...
    for e in entities:
        ...
        for j in js:
            ...
            for d in range(n_diff_gmm):
                idx = j - 1 - diff_gmm0 - d
                zr[d] = y[idx] if idx >= 0 else 0.0
            zr[n_diff_gmm] = x[j]-x[j-1]; zr[n_diff_gmm+1] = z[j]-z[j-1]
            Zd.append(zr); E.append(e)
        for j in js:
            Yl.append(y[j]); Xl.append([y[j-1], x[j], z[j], 1.0])
            zr = np.zeros(nz); base = n_diff_gmm + 2
            if level_gmm_form == "D":
                zr[base] = y[j-1]-y[j-2] if j-2>=0 else 0.0
                if level_gmm_n >= 2: zr[base+1] = y[j-2]-y[j-3] if j-3>=0 else 0.0
            else:
                zr[base] = y[j-1] if j-1>=0 else 0.0
                if level_gmm_n >= 2: zr[base+1] = y[j-2] if j-2>=0 else 0.0
            zr[base+level_gmm_n] = x[j]; zr[base+level_gmm_n+1] = z[j]; zr[base+level_gmm_n+2] = 1.0
            Zl.append(zr); E.append(e)
    Y = np.array(Yd + Yl); X = np.array(Xd + Xl); Z = np.array(Zd + Zl); E = np.array(E)
    return Y, X, Z, E
...
def build_H_block(E, k, m0=0.0):
    ...
    n = len(E); H = np.zeros((n, n)); ents = E.tolist(); idx = 0
    while idx < n:
        e = ents[idx]; j = idx
        while j < n and ents[j] == e: j += 1
        kk = (j - idx) // 2
        M = np.eye(kk) - np.eye(kk, k=1)
        if m0 != 1.0: M[0, 0] = m0
        MM = M @ M.T; I = np.eye(kk)
...
for (dg0, ndg, lgn, form) in [(1,4,2,'D'), (2,3,2,'D')]:
    ...
    for m0 in [0.0, 1.0]:
        ...
        print("NI dg0=%d ndg=%d lgn=%d form=%s m0=%.1f ZHZvsA1inv rel=%.5f Zcols=%d" %
