...
for label, (step, robust) in [
    ...
    (ar1_pre, ar2_pre) = _ar_test(
        e, Z_by_entity, X, T_ent, step, robust, m2_raw, V_post, s2
    )
    # Test with post-small m2VZXA (current OE)
    (ar1_post, ar2_post) = _ar_test(
        e, Z_by_entity, X, T_ent, step, robust, m2_scaled, V_post, s2
    )

    s_ar1 = S[f"ar1_{label}"]
    s_ar2 = S[f"ar2_{label}"]

    print(
        f"  Pre-small m2VZXA:  AR1={ar1_pre[0]:.10f} diff={abs(ar1_pre[0] - s_ar1):.2e} | AR2={ar2_pre[0]:.10f} diff={abs(ar2_pre[0] - s_ar2):.2e}"
    )
    print(
        f"  Post-small m2VZXA: AR1={ar1_post[0]:.10f} diff={abs(ar1_post[0] - s_ar1):.2e} | AR2={ar2_post[0]:.10f} diff={abs(ar2_post[0] - s_ar2):.2e}"
    )
