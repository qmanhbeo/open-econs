...
for ei, e_val in enumerate(entities):
    ...
    for t in range(Ti):
        ...
        if t >= 1:
            Y_sys[dr] = y[t] - y[t - 1]
        if t >= 2:
            X_sys[dr, 0] = y[t - 1] - y[t - 2]
            X_sys[dr, 1] = xs[x_cols[0]][t] - xs[x_cols[0]][t - 1]
            X_sys[dr, 2] = xs[x_cols[1]][t] - xs[x_cols[1]][t - 1]
            Z_sys[dr, 2] = X_sys[dr, 1]
            Z_sys[dr, 3] = X_sys[dr, 2]
            Z_sys[dr, 5] = y[t - 2]
        if t >= 3:
            Z_sys[dr, 7] = y[t - 3]
        if t >= 4:
        ...
        lr = base + Ti + t
        Y_sys[lr] = y[t]
        X_sys[lr, 0] = y[t - 1] if t >= 1 else 0
        X_sys[lr, 1] = xs[x_cols[0]][t]
        X_sys[lr, 2] = xs[x_cols[1]][t]
        X_sys[lr, 3] = 1.0
        if t >= 1:
            Z_sys[lr, 0] = xs[x_cols[0]][t]
            Z_sys[lr, 1] = xs[x_cols[1]][t]
            Z_sys[lr, 4] = 1.0
        if t >= 2:
            Z_sys[lr, 6] = y[t - 1] - y[t - 2]
        if t >= 3:
            Z_sys[lr, 10] = y[t - 2] - y[t - 3]
    ...
    for j in range(min_j, T):
        ...
        for lag in depths:
            ...
            if idx >= 0:
                ...
                col += 1
        for iv_c in x_cols:
            Z_coll[r, col] = xs[iv_c][j] - xs[iv_c][j - 1]
            col += 1
...
def diff_ar_vecs(b, entities, y_by_e, x_by_e, x_cols, lags, min_j):
    ...
    for e_val in entities:
        ...
        xs = x_by_e[e_val]
        Ti = len(y_e)
        Xi = np.zeros((Ti, 1 + len(x_cols)))
        Yi = np.zeros(Ti)
        for j in range(1, Ti):
            col = 0
            for lag in range(1, lags + 1):
                if j - lag >= 1:
                    Xi[j, col] = y_e[j - lag] - y_e[j - lag - 1]
                elif j - lag == 0:
                    Xi[j, col] = y_e[0]
                col += 1
            for c in x_cols:
                Xi[j, col] = xs[c][j] - xs[c][j - 1]
                col += 1
            Yi[j] = y_e[j] - y_e[j - 1]
        e_i = Yi - Xi @ b
        e_i[:min_j] = 0
...
for label, (step, robust) in [
    ...
    print(
        f"  Sys b:  {est_sys['b'][0]:.8f} {est_sys['b'][1]:.8f} {est_sys['b'][2]:.8f} {est_sys['b'][3]:.8f}"
    )
    print(
        f"  Diff b: {est_diff['b'][0]:.8f} {est_diff['b'][1]:.8f} {est_diff['b'][2]:.8f}"
    )

    # AR vectors from system estimation
    e_s, X_s, T_s = system_ar_vecs(
        est_sys["b"], entities, y_by_e, x_by_e, x_cols, min_j, T
    )
    # AR vectors from diff-only estimation
    e_d, X_d, T_d = diff_ar_vecs(
        est_diff["b"], entities, y_by_e, x_by_e, x_cols, lags, min_j
    )
    ...
    dr = np.zeros(int(N_d_v))
    idx = 0
    for ent_idx in range(n_ent):
        base = ent_idx * N_ROW_PER
        for t in range(2, T):
            dr[idx] = est_sys["e"][base + t]
            idx += 1
    s2 = float(dr @ dr) / N_d_v / 2.0
    s2 *= N_d_v / (N_d_v - k)
    if onestep:
        r = s2 / est_sys["sig2"]
        Vp = est_sys["pV"] * r
        ...
        Vp = est_sys["pV"] * rv
        m2 = est_sys["m2VZXA"] * sm
    (ar1_s, ar2_s) = _ar_test(
        e_s, Z_diff_sys_by_ent, X_s, T_s, step, robust, m2, Vp, s2
    )
    ...
    if onestep:
        ...
        Vp_d = est_diff["pV"] * rd
        m2_d = est_diff["m2VZXA"] * rd
    else:
        NObs_d = float(n_ent * (T - 1))
        wttot_d = float(len(Y_coll))
        rv_d = ((NObs_d - 1) / (NObs_d - kd)) / ((wttot_d - 1) / (wttot_d - kd))
        sm_d = ((NObs_d - 1) / (NObs_d - kd)) * (n_ent / (n_ent - 1))
        Vp_d = est_diff["pV"] * rv_d
        m2_d = est_diff["m2VZXA"] * sm_d
    (ar1_d, ar2_d) = _ar_test(
        e_d,
        Z_diff_coll_by_ent,
        X_d,
        T_d,
        ...
        m2_d,
        Vp_d,
        s2_d if onestep else est_diff["sig2"],
    )
