...
for label, (step, robust) in flavors.items():
    ...
    for vname, V in formulas:
        ...
        (ar1_d, ar2_d) = _ar_test(
            e, Z_diff_by_ent, X, T_ent, step, robust, m2_pre, V, s2_stata
        )
        d1d = abs(ar1_d[0] - a1_fix)
        d2d = abs(ar2_d[0] - a2_fix)
        ...
        if not onestep:
            ...
            for lag_val, res_list in [(1, None), (2, None)]:
                ...
                for ent in e:
                    ...
                    s = float(e_e @ wli)
                    whw += s**2
                    ZHw_v += Z_full_by_ent[ent].T @ e_full_2t[ent] * s
                    tmp_v += X[ent].T @ wli
                    sum_t += s
                dnom = np.sqrt(whw + tmp_v @ (m2_pre @ ZHw_v + V @ tmp_v))
                if lag_val == 1:
                    ar1_f = sum_t / dnom if dnom > 0 else float("nan")
                else:
                    ar2_f = sum_t / dnom if dnom > 0 else float("nan")
            d1f = abs(ar1_f - a1_fix)
            d2f = abs(ar2_f - a2_fix)
