...
for tau in range(T - 1):
    M_fwd[tau, tau + 1] = -1.0
W = block_diag(
    *[np.block([[M_fwd.T @ M_fwd, M_fwd.T], [M_fwd, np.eye(T)]]) for _ in range(n_ent)]
)
...
print(
    f"{'Flavor':8s} {'Method':>15s} {'AR1':>12s} {'AR1_gap':>10s}  {'AR2':>12s} {'AR2_gap':>10s}"
)
print("-" * 75)
...
for label, (step, robust) in flavors.items():
    ...
    a1_fix = S[f"ar1_c_{label}"]
    a2_fix = S[f"ar2_c_{label}"]

    # Method 1: current OE (diff ZHw)
    (ar1_cur, ar2_cur) = _ar_test(
        e, Z_diff_by_ent, X, T_ent, step, robust, m2, V_post, s2
    )
    d1 = abs(ar1_cur[0] - a1_fix)
    d2 = abs(ar2_cur[0] - a2_fix)
    print(
        f"{label:8s} {'diff ZHw (curr)':>15s} {ar1_cur[0]:12.8f} {d1:10.2e}  {ar2_cur[0]:12.8f} {d2:10.2e}"
    )
    ...
    for method_name, use_full in [("full ZHw", True)]:
        for lag_idx, lag_val in enumerate([1, 2]):
            ...
            for ent in e:
                ...
                e_e = e[ent]
                wli = np.zeros(Ti)
                wli[lag_val:] = e_e[: Ti - lag_val]
                s = float(e_e @ wli)
                if onestep:
                    from open_econs.models.linear.abond import _build_H_ar

                    Hmat = _build_H_ar(Ti)
                    whw += float(wli @ Hmat @ wli) * s2
                    psiw = Hmat @ wli * s2
                    ZHw += Z_diff_by_ent[ent].T @ psiw
                    tmp += X[ent].T @ wli
                    sum_t += s
                else:
                    if not use_full:
                        whw += s**2
                        ZHw += Z_diff_by_ent[ent].T @ e_e * s
                        tmp += X[ent].T @ wli
                        sum_t += s
                    else:
                        whw += s**2
                        ZHw += Z_full_by_ent[ent].T @ e_full_2t[ent] * s
                        tmp += X[ent].T @ wli
                        sum_t += s
        ...
        d1n = abs(ar1_new - a1_fix)
        d2n = abs(ar2_new - a2_fix)
        print(
            f"{label:8s} {method_name:>15s} {ar1_new:12.8f} {d1n:10.2e}  {ar2_new:12.8f} {d2n:10.2e}"
        )
