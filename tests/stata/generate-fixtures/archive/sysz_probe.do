clear all
set more off
cd "C:\Users\manhn\Desktop\open-econs\tests\stata\generate-fixtures"
log using "sysz_probe.log", replace text

import delimited "fixtures/inputs/df_panel.csv", clear
xtset entity time
...
if _rc == 0 {
    ...
    matrix Zmat = e(Z)
    svmat double Zmat
    export delimited using "sys_Z.csv", replace
...
if _rc == 0 {
    matrix Xmat = e(X)
    svmat double Xmat
    export delimited using "sys_X.csv", replace
...
if _rc == 0 {
    matrix Ymat = e(Y)
    svmat double Ymat
    export delimited using "sys_Y.csv", replace
...
if _rc == 0 {
    matrix Hmat = e(H)
    svmat double Hmat
    export delimited using "sys_H.csv", replace
