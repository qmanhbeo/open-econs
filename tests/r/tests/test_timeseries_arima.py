...
# (genuine cross-tool absolute gap ~1.1e-5). 1e-4 absolute bounds it with margin.
ATOL_CONST = 1e-4
...
class TestARIMAR:
    ...
    def test_const(self):
        f = _fit()
        npt.assert_allclose(f["cons"], R_ARIMA["cons"], rtol=0, atol=ATOL_CONST)

    def test_ar1(self):
        f = _fit()
        npt.assert_allclose(f["ar1"], R_ARIMA["ar1"], rtol=RTOL_COEF)

    def test_ma1(self):
        f = _fit()
        npt.assert_allclose(f["ma1"], R_ARIMA["ma1"], rtol=RTOL_COEF)
